...
const newFlavors = copy(originalFlavors)
...
function removeFlavorByName(arr, flavor){
  let toRemove = arr.indexOf(flavor);
...
function filterByWord(arr, filterFlavor){
  let newArr = []
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].includes(filterFlavor)) {
      newArr.push(arr[i]);
    }
    return newArr;
  }
...
function getAverageWordLength(arr){
  ...
  for (let index of arr) {
    wordsInFlavor = index.split(' ');
    numOfWords = wordsInFlavor.length;
    if(numOfWords === 1) {
      arrToAvg.push(1);
    }
...
let average = sum / arrToAvg.length;
return average;
}
...
function getRandomFlavors(originalFlavorsArr, newFlavorsArr, seasonalFlavorsArr, regionalFlavorsArr) {
  const allFlavors = originalFlavors.concat(newFlavorsArr, seasonalFlavorsArr, regionalFlavorsArr);
  const randomFlavors = [];
  for (let i = 0; i < 31; i++) {
...
//console.log(getRandomFlavors(originalFlavors, newFlavors2, seasonalFlavors, regionalFlavors).length)
